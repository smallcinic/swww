...
try {
	...
	switch ($_GET['view']) {
		...
		case 'pages':
			...
			require('includes/class-pager.php');
			$params['selected_page'] = ( (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
			$total = $db->query("SELECT * FROM pages") or _error(SQL_ERROR);
			$params['total_items'] = $total->num_rows;
			$params['items_per_page'] = $system['max_results'];
		...
		case 'events':
			...
			require('includes/class-pager.php');
			$params['selected_page'] = ( (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
			$total = $db->query("SELECT * FROM events WHERE event_privacy = 'public'") or _error(SQL_ERROR);
			$params['total_items'] = $total->num_rows;
			$params['items_per_page'] = $system['max_results'];
			...
			while($row = $get_rows->fetch_assoc()) {
				$row['event_picture'] = $user->get_picture($row['event_cover'], 'event');
		...
		case 'games':
			...
			require('includes/class-pager.php');
			$params['selected_page'] = ( (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
			$total = $db->query("SELECT * FROM games") or _error(SQL_ERROR);
			$params['total_items'] = $total->num_rows;
			$params['items_per_page'] = $system['max_results'];
