...
class User {
    ...
    public function publisher($args = array()) {
        ...
        $post['post_author_blog_url'] = $system['system_url'].'/blog/'.$this->_data['user_name'];
	    ...
	    $post['post_author_blog_url'] = $system['system_url'].'/pages/'.$_page['page_name'];
    ...
    private function _check_post($id, $pass_privacy_check = false, $full_details = true) {
        ...
        if($post['user_type'] == "user") {
            ...
            $post['post_author_blog_url'] = $system['system_url'].'/blogs/'.$post['user_name'];
	    $post['post_author_name'] = $post['user_firstname']." ".$post['user_lastname'];
            ...
            $post['post_author_blogs_url'] = $system['system_url'].'/pages/'.$post['page_name'];
     ...
     * @return array
     */
    public function get_blog($username,$args = array()) {
        global $db, $system;
        /* initialize arguments */
        $offset = !isset($args['offset']) ? 0 : $args['offset'];
        $random = !isset($args['random']) ? false : true;
        ...
        /* get posts */
        $order_query = ($random) ? "ORDER BY RAND()" : "ORDER BY posts.post_id DESC";
	$get_posts = $db->query(sprintf("SELECT posts.post_id FROM posts INNER JOIN posts_articles ON posts.post_id = posts_articles.post_id INNER JOIN users ON posts.user_id = users.user_id WHERE posts.post_type = 'article' AND user_name = '$username' ".$order_query." LIMIT %s, %s", secure($offset, 'int', false), secure($results, 'int', false) )) or _error(SQL_ERROR_THROWEN);
        if($get_posts->num_rows > 0) {
            while($post = $get_posts->fetch_assoc()) {
                $post = $this->get_post($post['post_id']);
                if($post) {
