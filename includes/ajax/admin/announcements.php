...
try {
	...
	switch ($_GET['do']) {
		...
		case 'add':
			...
			/* insert */
			$db->query(sprintf("INSERT INTO announcements (name, title, type, code, start_date, end_date) VALUES (%s, %s, %s, %s, %s, %s)", secure($_POST['name']), secure($_POST['title']), secure($_POST['type']), secure($_POST['message']), secure($_POST['start_date'], 'datetime'), secure($_POST['end_date'], 'datetime') )) or _error(SQL_ERROR_THROWEN);
			/* return */
			return_json( array('callback' => 'window.location = "'.$system['system_url'].'/admincp/announcements";') );
			break;
