...
try {
	...
	switch ($_POST['handle']) {
		...
		case 'product':
			...
			if(!isset($_POST['name']) || !isset($_POST['price']) || !isset($_POST['category_id']) || !isset($_POST['location']) || !isset($_POST['message'])) {
				_error(400);
			}
			/* check product name */
			if(is_empty($_POST['name'])) {
				return_json( array('error' => true, 'message' => __("Add a name so people know what you are selling")) );
			}
			/* check product price */
			...
			if(!is_numeric($_POST['price']) || $_POST['price'] <= 0) {
				return_json( array('error' => true, 'message' => __("Please add valid product price")) );
			}
			/* check product category */
			if(!in_array($_POST['category_id'], $user->get_market_categories_ids())) {
				return_json( array('error' => true, 'message' => __("Please add valid product category")) );
			}
			
			// edit product
			$user->edit_product($_POST['id'], $_POST['message'], $_POST['name'], $_POST['price'], $_POST['category_id'], $_POST['location']);
			/* return */
