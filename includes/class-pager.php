...
class Pager {
    ...
    public function getPager() {
        ...
        // total pages
        $output = '<li class="disabled"><a>'.__("Displaying").' ('.$from.'-'.$to.' '.__("of").' '.$this->_total.')</a></li>';
        ...
        // go to first page
        if($loopStart != 1) {
            $output .= sprintf('<li><a href="' . $this->_link . '">&#171;</a></li>', '1');
        }
        
        // previous page
        if($this->_page > 1) {
            $output .= sprintf('<li><a href="' . $this->_link . '">'.__("Prev").'</a></li>', $this->_page - 1);
        }
        ...
        // next page
        if($this->_page < $totalPages) {
            $output .= sprintf('<li><a href="' . $this->_link . '">'.__("Next").'</a></li>', $this->_page + 1);
		}
