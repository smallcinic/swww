        ...
        <div class="footer">
            {if $user->_data['user_id'] != $profile['user_id']}
                ...
                <div class="btn-group">
                    ...
                    {/if}
                    <button type="button" class="btn btn-default js_chat-start" data-uid="{$profile['user_id']}" data-name="{$profile['user_firstname']} {$profile['user_lastname']}" data-picture="{$profile['user_picture']}">{__("Message")}</button>
                </div>
            {else}
